...
with c_map1:
    ...
    end_choice = st.selectbox("🏁 Chọn nhanh Điểm Đến:", ["🗺️ Tự nhập địa chỉ (Tìm kiếm)"] + list(QUICK_LOCATIONS.keys()), key="end_quick")
    if end_choice == "🗺️ Tự nhập địa chỉ (Tìm kiếm)":
    ...
    veh_dict = {
        "🏍️ Xe Máy Thường": {"rate": 6000, "budget": 2},
    ...
    if can_book:
        ...
        with col_rs2:
            ...
            if st.button(btn_text, use_container_width=True, type="primary"):
                ...
                if final_pay == 0:
                    earned_pts = 0
                    pt_msg = "⚠️ Chuyến đi 0đ KHÔNG áp dụng tích lũy điểm thưởng để chống trục lợi."
                else:
                ...
                if st.session_state.package and st.session_state.is_trial:
                    ...
                    st.warning("⚠️ Chuyến đi trải nghiệm KHÔNG áp dụng tích điểm. Gói dùng thử đã hết hạn.")
                else:
        ...
        st.markdown("<hr style='border: 1px dashed #ccc;'/>", unsafe_allow_html=True)
        st.subheader("🛍️ Siêu thị Đặc Quyền MonthlyGo")
        ...
        for idx, p_name in enumerate(ALL_PACKAGES):
            with cols[idx]:
                ...
                if st.button(f"🛒 Đăng ký Gói", key=f"buy_{p_name}", use_container_width=True):
                    ...
                    st.session_state.is_trial = False 
                    st.toast(f'Cảm ơn bạn đã mua {p_name}! Áp dụng đặc quyền tức thì.', icon='✅')
                    st.rerun()
                ...
                if p_name not in st.session_state.used_trials:
                    if st.button(f"🎁 Dùng thử", key=f"trial_{p_name}", use_container_width=True):
                        ...
                        st.toast(f'Đã kích hoạt 1 lượt dùng thử {p_name}!', icon='✅')
                        st.rerun()
                else:
